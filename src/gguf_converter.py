...
MODEL_TO_CONVERT_PATH = os.path.abspath(os.path.join(os.path.dirname(__file__), 'hyperlane-qwen2-merged-model'))
...
def run_command(command, cwd):
    ...
    try:
        ...
        print("--- ERROR ---")
        print(f"Command not found: {command[0]}. Make sure it is installed and in your PATH.")
        sys.exit(1)
...
def main():
    ...
    if not os.path.isdir(LLAMA_CPP_PATH):
        ...
        print("Please ensure the llama.cpp folder is inside the 'ai-training' directory.")
        sys.exit(1)
    ...
    run_command([python_executable, "-m", "pip", "install", "numpy", "sentencepiece", "protobuf", "mistral_common"], LLAMA_CPP_PATH)
    ...
    output_file_path = os.path.abspath(os.path.join(os.path.dirname(__file__), OUTPUT_GGUF_FILE))
    python_executable = sys.executable
